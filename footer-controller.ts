...
interface FooterSlideState {
  ...
  // Track last scroll position to infer scrollbar drag deltas
  lastScrollY: number;
  // Track if we're currently near bottom to handle scrollbar dragging
  wasAtBottom: boolean;
  ...
  return {
    ...
    initFooterSlide(this: FooterController, options: Partial<FooterSlideOptions> = {}) {
      ...
      const isAtBottom = (): boolean => {
        ...
        const threshold = 3; // Allow for small rounding errors
        return Math.ceil(window.scrollY + window.innerHeight) >= doc.scrollHeight - threshold;
      };

      const isNearBottom = (): boolean => {
        const doc = document.documentElement;
        const threshold = 50; // Consider "near bottom" within 50px
        return Math.ceil(window.scrollY + window.innerHeight) >= doc.scrollHeight - threshold;
      };
      ...
      const handleRevealDelta = (rawDeltaY: number): boolean => {
        ...
        const currentNearBottom = isNearBottom();
        
        // If we're not near the bottom and weren't at bottom, don't handle
        if (!currentNearBottom && !state.wasAtBottom) return false;

        const cap = getEffectiveMaxRevealPx();
        const goingDown = rawDeltaY > 0;
        const goingUp = rawDeltaY < 0;

        // If going down and already fully revealed, prevent default scroll
        if (goingDown && state.revealedPx >= cap - 0.1) return true;
        
        // If going up and footer is hidden, allow normal scroll unless we're at bottom
        if (goingUp && state.revealedPx <= 0.1) {
          return currentAtBottom; // Prevent scroll only if at bottom
        }
        ...
        if (goingDown) {
          ...
          setReveal(state.revealedPx - step);
          return state.revealedPx > 0.1 || currentAtBottom;
        }
        return false;
      };
      ...
      const onScroll = throttle(() => {
        ...
        // Update wasAtBottom state
        if (currentAtBottom) {
          state.wasAtBottom = true;
        } else if (!currentNearBottom) {
          state.wasAtBottom = false;
        }

        // Handle scroll delta if we're at bottom, near bottom, or were at bottom
        if (currentAtBottom || currentNearBottom || state.wasAtBottom) {
          if (delta !== 0) {
            handleRevealDelta(delta);
          }
          return;
        }

        // Not near bottom and wasn't at bottom: ensure footer is hidden
