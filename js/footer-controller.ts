  ...
  return {
    ...
    initFooterSlide(this: FooterController, options: Partial<FooterSlideOptions> = {}) {
      ...
      const onTouchMove = (e: TouchEvent) => {
        ...
        // Check if we're at the bottom or if footer is already partially revealed
        const atBottom = isAtBottom();
        const footerRevealing = state.revealedPx > 0.1;
        
        // If we're at bottom or footer is already revealing, handle the delta
        // This allows the footer to appear smoothly when pulling at the bottom, just like with wheel
        if (atBottom || footerRevealing) {
